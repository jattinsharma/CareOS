...
// Next dose date after a dose taken at `lastTaken`, for the app's frequency
// values ("Daily"/"Twice daily" = every day, "every-2-days", "every-3-days")
// and a local dose time "HH:MM". Returns a Date, or null for invalid input.
// Unrecognized frequencies fall back to daily so callers never crash.
export function calculateNextDose(lastTaken, frequency = "daily", time = "09:00") {
  const taken = lastTaken instanceof Date ? lastTaken : new Date(lastTaken);
  if (!taken || Number.isNaN(taken.getTime())) return null;
  const freq = String(frequency || "daily").toLowerCase();
  const intervalDays = freq === "every-2-days" ? 2 : freq === "every-3-days" ? 3 : 1;
